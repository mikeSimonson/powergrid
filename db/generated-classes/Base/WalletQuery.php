 ...
 * @method     ChildWalletQuery orderByBalance($order = Criteria::ASC) Order by the balance column
 *
 * @method     ChildWalletQuery groupById() Group by the id column
 * @method     ChildWalletQuery groupByBalance() Group by the balance column
 *
 ...
 *
 * @method     ChildWalletQuery leftJoinPlayer($relationAlias = null) Adds a LEFT JOIN clause to the query using the Player relation
 * @method     ChildWalletQuery rightJoinPlayer($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Player relation
 * @method     ChildWalletQuery innerJoinPlayer($relationAlias = null) Adds a INNER JOIN clause to the query using the Player relation
 *
 ...
 * @method     ChildWallet findOneByBalance(int $balance) Return the first ChildWallet filtered by the balance column *
 ...
 * @method     ChildWallet requireOneByBalance(int $balance) Return the first ChildWallet filtered by the balance column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 *
 ...
 * @method     ChildWallet[]|ObjectCollection findByBalance(int $balance) Return ChildWallet objects filtered by the balance column
...
abstract class WalletQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'powergrid', $modelName = '\\Wallet', $modelAlias = null)
    {
    ...
    public function findPk($key, ConnectionInterface $con = null)
    {
        ...
        if ((null !== ($obj = WalletTableMap::getInstanceFromPool((string) $key))) && !$this->formatter) {
    ...
    {
        $sql = 'SELECT id, balance FROM wallet WHERE id = :p0';
        try {
        ...
        if ($row = $stmt->fetch(\PDO::FETCH_NUM)) {
            ...
            $obj = new ChildWallet();
            $obj->hydrate($row);
            WalletTableMap::addInstanceToPool($obj, (string) $key);
        }
        $stmt->closeCursor();
    ...
    public function filterByBalance($balance = null, $comparison = null)
    {
        if (is_array($balance)) {
            ...
            if (isset($balance['min'])) {
                $this->addUsingAlias(WalletTableMap::COL_BALANCE, $balance['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($balance['max'])) {
                $this->addUsingAlias(WalletTableMap::COL_BALANCE, $balance['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(WalletTableMap::COL_BALANCE, $balance, $comparison);
    }
    ...
    public function filterByPlayer($player, $comparison = null)
    {
        if ($player instanceof \Player) {
            return $this
                ->addUsingAlias(WalletTableMap::COL_ID, $player->getWalletId(), $comparison);
