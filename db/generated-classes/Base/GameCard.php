...
abstract class GameCard implements ActiveRecordInterface
{
    ...
    /**
     * The value for the card_status_id field.
     * @var        int
     */
    protected $card_status_id;
    ...
    /**
     * Get the [card_status_id] column value.
     *
     * @return int
     */
    ...
    /**
     * Set the value of [card_status_id] column.
     *
     * @param int $v new value
     * @return $this|\GameCard The current object (for fluent API support)
     */
    public function setCardStatusId($v)
    {
        if ($v !== null) {
            $v = (int) $v;
        }

        if ($this->card_status_id !== $v) {
            $this->card_status_id = $v;
            $this->modifiedColumns[GameCardTableMap::COL_CARD_STATUS_ID] = true;
        }

        if ($this->aCardStatus !== null && $this->aCardStatus->getId() !== $v) {
            $this->aCardStatus = null;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : GameCardTableMap::translateFieldName('GameId', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : GameCardTableMap::translateFieldName('CardStatusId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->card_status_id = (null !== $col) ? (int) $col : null;
            $this->resetModified();
    ...
    public function ensureConsistency()
    {
        ...
        if ($this->aCardStatus !== null && $this->card_status_id !== $this->aCardStatus->getId()) {
            $this->aCardStatus = null;
        }
    } // ensureConsistency
    ...
    protected function doSave(ConnectionInterface $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->aCardStatus !== null) {
                if ($this->aCardStatus->isModified() || $this->aCardStatus->isNew()) {
                    $affectedRows += $this->aCardStatus->save($con);
                }
                $this->setCardStatus($this->aCardStatus);
            }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(GameCardTableMap::COL_CARD_STATUS_ID)) {
            $modifiedColumns[':p' . $index++]  = 'card_status_id';
        }
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case 'card_id':
                        ...
                        break;
                    case 'card_status_id':
                        $stmt->bindValue($identifier, $this->card_status_id, PDO::PARAM_INT);
                        break;
                }
            }
            $stmt->execute();
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if ($includeForeignObjects) {
            ...
            if (null !== $this->aCardStatus) {
                ...
                $result[$key] = $this->aCardStatus->toArray($keyType, $includeLazyLoadColumns,  $alreadyDumpedObjects, true);
            }
        }
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[2], $arr)) {
            $this->setCardStatusId($arr[$keys[2]]);
        }
    }
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(GameCardTableMap::COL_CARD_STATUS_ID)) {
            $criteria->add(GameCardTableMap::COL_CARD_STATUS_ID, $this->card_status_id);
        }
    ...
    public function hashCode()
    {
        $validPk = null !== $this->getGameId() &&
            null !== $this->getCardId() &&
            null !== $this->getCardStatusId();

        $validPrimaryKeyFKs = 3;
        $primaryKeyFKs = [];

        //relation game_card_fk_3cbcd5 to table game
        ...
        //relation game_card_fk_acfa40 to table card
        ...
        //relation game_card_fk_75e1e3 to table card_status
        if ($this->aCardStatus && $hash = spl_object_hash($this->aCardStatus)) {
            $primaryKeyFKs[] = $hash;
        } else {
            $validPrimaryKeyFKs = false;
        }
    ...
    {
        return (null === $this->getGameId()) && (null === $this->getCardId()) && (null === $this->getCardStatusId());
    }
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        $copyObj->setCardStatusId($this->getCardStatusId());
        if ($makeNew) {
            $copyObj->setNew(true);
        }
    }
    ...
    /**
     * Declares an association between this object and a ChildCardStatus object.
     *
     * @param  ChildCardStatus $v
     * @return $this|\GameCard The current object (for fluent API support)
     * @throws PropelException
     */
    public function setCardStatus(ChildCardStatus $v = null)
    {
        if ($v === null) {
            $this->setCardStatusId(NULL);
        } else {
            $this->setCardStatusId($v->getId());
        }

        $this->aCardStatus = $v;

        // Add binding for other direction of this n:n relationship.
        // If this object has already been added to the ChildCardStatus object, it will not be re-added.
        if ($v !== null) {
            $v->addGamecard($this);
        }
    ...
    /**
     * Get the associated ChildCardStatus object
     *
     * @param  ConnectionInterface $con Optional Connection object.
     * @return ChildCardStatus The associated ChildCardStatus object.
     * @throws PropelException
     */
    public function getCardStatus(ConnectionInterface $con = null)
    {
        if ($this->aCardStatus === null && ($this->card_status_id !== null)) {
            $this->aCardStatus = ChildCardStatusQuery::create()->findPk($this->card_status_id, $con);
            /* The following can be used additionally to
                guarantee the related object contains a reference
                to this object.  This level of coupling may, however, be
                undesirable since it could result in an only partially populated collection
                in the referenced object.
                $this->aCardStatus->addGamecards($this);
             */
        }

        return $this->aCardStatus;
    }
    ...
    public function clear()
    {
        ...
        if (null !== $this->aCardStatus) {
            $this->aCardStatus->removeGamecard($this);
        }
        $this->game_id = null;
        $this->card_id = null;
        $this->card_status_id = null;
        $this->alreadyInSave = false;
        $this->clearAllReferences();
        $this->resetModified();
        $this->setNew(true);
