 ...
 *
 * @method     \PlayerQuery|\GameCardQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
...
abstract class PlayerCardQuery extends ModelCriteria
{
     ...
     * @return ChildPlayerCardQuery The current query, for fluid interface
     */
    public function filterByCard($gameCard, $comparison = null)
    {
        if ($gameCard instanceof \GameCard) {
            return $this
                ->addUsingAlias(PlayerCardTableMap::COL_CARD_ID, $gameCard->getId(), $comparison);
            ...
            return $this
                ->addUsingAlias(PlayerCardTableMap::COL_CARD_ID, $gameCard->toKeyValue('PrimaryKey', 'Id'), $comparison);
        } else {
            throw new PropelException('filterByCard() only accepts arguments of type \GameCard or Collection');
        }
    }
    ...
    /**
     * Use the Card relation GameCard object
     *
     * @see useQuery()
     *
     * @param     string $relationAlias optional alias for the relation,
    ...
    public function useCardQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
    {
        return $this
            ...
            ->useQuery($relationAlias ? $relationAlias : 'Card', '\GameCardQuery');
    }
