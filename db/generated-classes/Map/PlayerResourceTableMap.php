...
/**
 * This class defines the structure of the 'player_resource' table.
 *
 *
 *
 ...
 *
 */
class PlayerResourceTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = '.Map.PlayerResourceTableMap';
     ...
     */
    const TABLE_NAME = 'player_resource';

    /**
     * The related Propel class for this table
     */
    const OM_CLASS = '\\PlayerResource';

    /**
     * A class that can be returned by this tableMap
     */
    const CLASS_DEFAULT = 'PlayerResource';
     ...
     */
    const COL_QUANTITY = 'player_resource.quantity';
    ...
    {
        // attributes
        $this->setName('player_resource');
        $this->setPhpName('PlayerResource');
        $this->setIdentifierQuoting(false);
        $this->setClassName('\\PlayerResource');
        $this->setPackage('');
  ...
  array (
     ...
     * and findPk*() calls.
     *
     * @param \PlayerResource $obj A \PlayerResource object.
     ...
     *
     * @param mixed $value A \PlayerResource object or a primary key value.
     */
    public static function removeInstanceFromPool($value)
    {
        if (Propel::isInstancePoolingEnabled() && null !== $value) {
            if (is_object($value) && $value instanceof \PlayerResource) {
                ...
                $e = new PropelException("Invalid value passed to removeInstanceFromPool().  Expected primary key or \PlayerResource object; got " . (is_object($value) ? get_class($value) . ' object.' : var_export($value, true)));
                throw $e;
            }
    ...
    {
        return $withPrefix ? PlayerResourceTableMap::CLASS_DEFAULT : PlayerResourceTableMap::OM_CLASS;
    }
     ...
     * @return array           (PlayerResource object, last column rank)
     */
    public static function populateObject($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        $key = PlayerResourceTableMap::getPrimaryKeyHashFromRow($row, $offset, $indexType);
        if (null !== ($obj = PlayerResourceTableMap::getInstanceFromPool($key))) {
            // We no longer rehydrate the object, since this can cause data loss.
            // See http://www.propelorm.org/ticket/509
            // $obj->hydrate($row, $offset, true); // rehydrate
            $col = $offset + PlayerResourceTableMap::NUM_HYDRATE_COLUMNS;
        } else {
            $cls = PlayerResourceTableMap::OM_CLASS;
            /** @var PlayerResource $obj */
            $obj = new $cls();
            $col = $obj->hydrate($row, $offset, false, $indexType);
            PlayerResourceTableMap::addInstanceToPool($obj, $key);
        }
    ...
    public static function populateObjects(DataFetcherInterface $dataFetcher)
    {
        ...
        while ($row = $dataFetcher->fetch()) {
            $key = PlayerResourceTableMap::getPrimaryKeyHashFromRow($row, 0, $dataFetcher->getIndexType());
            if (null !== ($obj = PlayerResourceTableMap::getInstanceFromPool($key))) {
                // We no longer rehydrate the object, since this can cause data loss.
                // See http://www.propelorm.org/ticket/509
    ...
    {
        return Propel::getServiceContainer()->getDatabaseMap(PlayerResourceTableMap::DATABASE_NAME)->getTable(PlayerResourceTableMap::TABLE_NAME);
    }
    ...
    {
        $dbMap = Propel::getServiceContainer()->getDatabaseMap(PlayerResourceTableMap::DATABASE_NAME);
        if (!$dbMap->hasTable(PlayerResourceTableMap::TABLE_NAME)) {
            $dbMap->addTableObject(new PlayerResourceTableMap());
        }
    }

    /**
     * Performs a DELETE on the database, given a PlayerResource or Criteria object OR a primary key value.
     *
     * @param mixed               $values Criteria or PlayerResource object or primary key or array of primary keys
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \PlayerResource) { // it's a model object
        ...
        $query = PlayerResourceQuery::create()->mergeWith($criteria);

        if ($values instanceof Criteria) {
            PlayerResourceTableMap::clearInstancePool();
        } elseif (!is_object($values)) { // it's a primary key, or an array of pks
            foreach ((array) $values as $singleval) {
                PlayerResourceTableMap::removeInstanceFromPool($singleval);
            }
        }
    ...
    {
        return PlayerResourceQuery::create()->doDeleteAll($con);
    }

    /**
     * Performs an INSERT on the database, given a PlayerResource or Criteria object.
     *
     * @param mixed               $criteria Criteria or PlayerResource object containing data that is used to create the INSERT statement.
    ...
    public static function doInsert($criteria, ConnectionInterface $con = null)
    {
        ...
        if ($criteria instanceof Criteria) {
            ...
            $criteria = $criteria->buildCriteria(); // build Criteria from PlayerResource object
        }
...
} // PlayerResourceTableMap
// This is the static code needed to register the TableMap for this table with the main Propel class.
//
PlayerResourceTableMap::buildTableMap();
